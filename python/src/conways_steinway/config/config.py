...
class GenerationLimit:
    ...
    def __init__(self, value: str | int | dict[str, int] = UNLIMITED) -> None:
...
class Config:
    ...
    def from_args_and_env(cls) -> "Config":
        ...
        parser = argparse.ArgumentParser(
            description="Conway's Steinway - Game of Life Piano Generator"
        )
        parser.add_argument("--config", type=str, help="Path to configuration file")
        parser.add_argument(
            "--board-type",
            type=str,
            choices=["random", "static", "fur_elise", "complex", "showcase"],
            help="Board initialization type",
        )
        parser.add_argument(
            "--silent", action="store_true", help="Disable audio output"
        )
        # Remove --audio flag since audio is now the default and we only check for --silent
        parser.add_argument(
            "--generations", type=str, help='Generation limit (number or "Unlimited")'
        )
        parser.add_argument(
            "--step-delay", type=int, help="Delay between steps in milliseconds"
        )
        parser.add_argument(
            "--tempo", type=float, help="Musical tempo in beats per minute"
        )
        ...
        parser.add_argument(
            "--note-duration",
            type=int,
            dest="note_duration_ms",
            help="Duration of individual notes in milliseconds",
        )
        parser.add_argument(
            "--gap", type=int, dest="gap_ms", help="Gap between notes in milliseconds"
        )
        parser.add_argument(
            "--chord-duration",
            type=int,
            dest="chord_duration_ms",
            help="Duration of chords in milliseconds",
        )
        parser.add_argument(
            "--initial-delay",
            type=int,
            dest="initial_delay_ms",
        ...
        # Board dimensions
        parser.add_argument(
            "--height", type=int, dest="board_height", help="Board height in cells"
        )
        ...
        if "CONWAYS_STEINWAY_BOARD_TYPE" in os.environ:
            config.board_type = BoardType.from_string(
                os.environ["CONWAYS_STEINWAY_BOARD_TYPE"]
            )
        ...
        if "CONWAYS_STEINWAY_GENERATIONS" in os.environ:
            config.generations = GenerationLimit(
                os.environ["CONWAYS_STEINWAY_GENERATIONS"]
            )
        ...
        if "CONWAYS_STEINWAY_CHORD_DURATION" in os.environ:
            config.chord_duration_ms = int(
                os.environ["CONWAYS_STEINWAY_CHORD_DURATION"]
            )
        ...
        if "CONWAYS_STEINWAY_ALIVE_PROBABILITY" in os.environ:
            config.alive_probability = float(
                os.environ["CONWAYS_STEINWAY_ALIVE_PROBABILITY"]
            )
    ...
    def load_from_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            if "audio.note.duration.ms" in properties:
                self.note_duration_ms = int(
                    properties.get("audio.note.duration.ms").data
                )
            ...
            if "audio.chord.duration.ms" in properties:
                self.chord_duration_ms = int(
                    properties.get("audio.chord.duration.ms").data
                )

            if "audio.initial.delay.ms" in properties:
                self.initial_delay_ms = int(
                    properties.get("audio.initial.delay.ms").data
                )
    ...
    def __str__(self) -> str:
        ...
        if self.board_type == BoardType.RANDOM:
            result += (
                f"  Random Board: {self.alive_probability * 100:.1f}% alive cells\n"
            )
