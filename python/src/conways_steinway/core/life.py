...
class Row:
    ...
    def __init__(
        self, cells: list[int] | None = None, width: int = BOARD_WIDTH
    ) -> None:
        if cells is None:
...
class Life:
    ...
    def new_random_board(
        ...
        for _ in range(height):
            row_cells = [
                # random.random() used for game mechanics, not security  # nosec B311
                Cell.ALIVE if random.random() < alive_probability else Cell.DEAD
    ...
    def from_pattern(self, pattern: list[str]) -> "Life":
        """
    ...
    def get_bottom_row_and_advance(self) -> list[int]:
        """
    ...
    def add_random_top_row(self) -> None:
        """
