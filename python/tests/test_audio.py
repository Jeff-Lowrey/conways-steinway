...
from src.conways_steinway.audio.piano import AudioEngine, AudioPlayer, NullAudioEngine


def test_audio_player_interface() -> None:
...
def test_null_audio_engine() -> None:
...
def test_audio_engine_chord_detection() -> None:
...
def test_audio_engine_key_to_note_name() -> None:
    ...
    assert (
        engine._key_to_note_name(51) == "C4"
    )  # Middle C is actually 51 in our mapping
    assert (
        engine._key_to_note_name(87) == "C7"
    )  # Highest piano key in our implementation
