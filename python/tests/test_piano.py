...
from src.conways_steinway.audio.piano import AudioPlayer, NullAudioEngine, Piano
from src.conways_steinway.core.life import Life, Row


def test_piano_initialization() -> None:
...
def test_piano_mute() -> None:
    """Test the mute functionality of the Piano class."""
    piano = Piano(audio_enabled=False)
    assert not piano.get_mute_status()

    piano.set_mute(True)
    assert piano.get_mute_status()

    piano.set_mute(False)
    assert not piano.get_mute_status()


def test_piano_row_to_keys() -> None:
...
class TestAudioEngine(NullAudioEngine):
    ...
    def __init__(self) -> None:
        self.keys_played: list[list[int]] = []
        self.chords_played: list[tuple[list[int], int]] = []
        self.sequences_played: list[tuple[list[int], int, int]] = []
...
def test_piano_play() -> None:
    ...
    piano._play_generation = mock_play_generation  # type: ignore
...
def test_piano_play_chord() -> None:
...
def test_piano_play_sequence() -> None:
